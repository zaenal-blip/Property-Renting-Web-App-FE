...
> = {
  ...
  CANCELLED: {
    label: "Cancelled",
    color: "bg-destructive/10 text-destructive border-destructive/20 text-[11px]",
    icon: XCircle,
  },
  COMPLETED: {
    label: "Completed",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Review and manage guest reservations for your properties.</p>
        </div>
      ...
      <div className="grid gap-6">
        {isLoading ? (
          ...
          <Card className="flex flex-col items-center justify-center py-20 text-center border-dashed">
              ...
              {(searchQuery || statusFilter !== "ALL") && (
                  <Button variant="outline" size="sm" className="mt-4" onClick={() => { setStatusFilter("ALL"); setSearchQuery(""); }}>
                      Clear filters
                  </Button>
              )}
          </Card>
          ...
          <div className="grid gap-4">
            {reservations.map((res: any) => {
              ...
              return (
                <Card key={res.id} className="overflow-hidden hover:shadow-sm transition-all border-border/50">
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="bg-muted/20 p-4 md:w-52 flex flex-col justify-center border-b md:border-b-0 md:border-r border-border">
                      <Badge className={`w-fit mb-2 ${config.color} border gap-1.5`}>
                        <StatusIcon className="h-3.5 w-3.5" />
                        {config.label}
                      </Badge>
                      <p className="text-[10px] text-muted-foreground font-mono uppercase tracking-tighter">ORDER: {res.id.slice(0,8)}</p>
                      <p className="text-base font-bold mt-1 text-primary">{formatPrice(res.totalPrice)}</p>
                    </div>
                    
                    {/* Info Section */}
                    <div className="flex-1 p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 items-center">
                      <div>
                         <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-1">Property</p>
                         <p className="font-bold text-sm leading-tight">{res.property?.name}</p>
                         <p className="text-[11px] text-muted-foreground mt-0.5">{res.reservationRooms?.[0]?.room?.name || "Room"}</p>
                      </div>
                      <div>
                         <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-1">Customer</p>
                         <p className="font-bold text-sm leading-tight">{res.user?.name}</p>
                         <p className="text-[11px] text-muted-foreground mt-0.5">{res.user?.email}</p>
                      </div>
                      <div>
                         <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-1">Schedule</p>
                         <div className="flex items-center gap-2">
                           <p className="font-bold text-xs">{new Date(res.checkinDate).toLocaleDateString()}</p>
                           <ArrowRight className="h-3 w-3 text-muted-foreground" />
                           <p className="font-bold text-xs">{new Date(res.checkoutDate).toLocaleDateString()}</p>
                         </div>
                      </div>
                    </div>
      ...
      <Dialog open={!!selectedProof} onOpenChange={(open) => !open && setSelectedProof(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="bg-muted rounded-xl overflow-hidden aspect-[3/4] flex items-center justify-center border">
            {selectedProof && (
                <img src={selectedProof} alt="Proof" className="max-h-full w-full object-contain" />
            )}
          </div>
          <DialogFooter>
            <Button className="w-full" onClick={() => setSelectedProof(null)}>Close Image</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm">
              {confirmModal?.action === "APPROVE" && "Approve this payment? The room will be locked for these dates and the guest will receive an email."}
              {confirmModal?.action === "REJECT" && "Rejecting will reset the status to Waiting Payment. The guest must upload a new proof."}
              {confirmModal?.action === "CANCEL" && "Are you sure? This will cancel the reservation permanently."}
            </DialogDescription>
          </DialogHeader>
